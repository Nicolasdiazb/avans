...
class WebGLWebViewDemo : MonoBehaviour {
    ...
    public CanvasWebViewPrefab AddressBar2;
    public CanvasWebViewPrefab MainWebView2;
    ...
    async void Start() {
        ...
        await AddressBar2.WaitUntilInitialized();
        ...
        AddressBar2.WebView.MessageEmitted += (sender, eventArgs) => {
            var message = eventArgs.Value;
            var messagePrefix = "url_submitted:";
            if (message.StartsWith(messagePrefix))
            {
                var url = message.Substring(messagePrefix.Length);
                MainWebView2.WebView.LoadUrl(url);
                StartCoroutine(_testIFrameUrl(url));
            }
        };
        ...
        await AddressBar2.WebView.WaitForNextPageLoadToFinish();

        // Initialize the address bar with the initial URL.
        AddressBar.WebView.PostMessage($"set_url:" + MainWebView.InitialUrl);
        AddressBar2.WebView.PostMessage($"set_url:" + MainWebView.InitialUrl);
    }
